...
def get_s(drop_days, roll_mat_csr, sales):
...
def get_w(sale_usd, roll_mat_csr):
    """
    """
...
def main():
    ...
    # Sales quantities:
    sales = pd.read_csv(os.path.join(data_pass, 'sales_train_validation.csv'))

    # Calendar to get week number to join sell prices:
    calendar = pd.read_csv(os.path.join(data_pass,'calendar.csv'))
    calendar = reduce_mem_usage(calendar)

    # Sell prices to calculate sales in USD:
    sell_prices = pd.read_csv(os.path.join(data_pass,'sell_prices.csv'))
    ...
    # Dump roll matrix to pickle: if you need to dumpy, uncomment code below 
    ...
    S = get_s(0, roll_mat_csr, sales)
    W = get_w(data[['id','sale_usd']], roll_mat_csr)
    ...
    score = wrmsse(sub, y_true, S, W, SW, roll_mat_csr, score_only=False)
    print(f'score: {score}')
