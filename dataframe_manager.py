...
def make_pca(df: pd.DataFrame, pca_col: str, n_days: int, keep_dim: int = 3):
    ...
    pca_df = pca_df.assign(
        **{
            format_s.format(col, l): pca_df.groupby([pca_col])[col].transform(
                lambda x: x.shift(l)
            )
            for l in LAG_DAYS
            for col in [TARGET]
...
def merge_by_concat(df1: pd.DataFrame, df2: pd.DataFrame, merge_on: list):
...
def price_features(prices_df: pd.DataFrame, calendar_df: pd.DataFrame):
    """
...
def sales_features(
    df: pd.DataFrame, mean_windows: list, std_windows: list,
):
    """
    For unit item
    Args:
    ...
    for window in mean_windows:
        df["rolling_price_mean_t" + str(window)] = df["sell_price"].transform(
            lambda x: x.rolling(window).mean()
        )

    for window in std_windows:
        df["rolling_price_std_t" + str(window)] = df["sell_price"].transform(
            lambda x: x.rolling(window).std()
        )
        df["rolling_mean_t" + str(window)] = df["demand"].transform(
            lambda x: x.rolling(window).mean()
        )

    for window in std_windows:
        df["rolling_std_t" + str(window)] = df["demand"].transform(
            lambda x: x.rolling(window).std()
        )
...
def demand_features(
    df: pd.DataFrame, shift: int, mean_windows: list, std_windows: list,
):
    """
    Args:
        df: pd.DataFrame
    ...
    for window in std_windows:
        df["rolling_std_t" + str(window)] = df["demand"].transform(
            lambda x: x.shift(shift).rolling(window).std()
        )
